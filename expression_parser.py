...
class ReadString(State):
    def transition(self, input_stream):
        for idx, c in enumerate(input_stream):
            if c.isalpha() or c.isnumeric() or c == "_":
                ...
                return self.StateOutput(
                    self.StateData(StateCode.Identifier, self.buffer),
                    input_stream[idx + 1 :],
                    ReadIndex(),
                )
            elif c == ".":
        ...
        return self.StateOutput(
            self.StateData(StateCode.Identifier, self.buffer), "", None
        )
...
class ReadIndex(State):
    def transition(self, input_stream):
        ...
        for idx, c in enumerate(input_stream):
            ...
            if (
                (idx == 1 and c == "x" and self.buffer == "0")
                or (hex_mode and c.isalpha())
                or c.isnumeric()
            ):
                self.buffer += c
            elif c == "]":
                return self.StateOutput(
                    self.StateData(StateCode.IndexAccess, self.buffer),
                    input_stream[idx + 1 :],
                    ReadString(),
                )
            else:
...
def parse_expression(expr):
    ...
    while True:
        state_data, remaining_input, current_state = current_state.transition(
            remaining_input
        )
        if state_data.value:
